...
//         strcpy(command, command_line[k]);
        ...
        if(stricmp(command, "printDebug") == 0){
            debug = 1;
            if(k + 1 >= nrhs || !mxIsNumeric(prhs[k + 1])){
                printf("Please input an integer\n");
                mxFree(command);
                break;
            }
            int num = (int)mxGetScalar(prhs[++k]);
            if(num >= 0 && num <= 9){
                debug = num;
                k++;
            }else{
        ...
        //original param 's' or 'S', default 's',
            ...
            if(k + 1 < nrhs && mxIsNumeric(prhs[k + 1])){
                ...
                //plot spectrum for each reading
                    ...
                    printf("wrong parameter for 'showSpectrum'");
                    mxFree(command);
                    break;
                }
            }
        //COM port, original parameter 'c'
            ...
            if(k + 1 >= nrhs || !mxIsNumeric(prhs[k + 1])){
                printf("Please input an integer follow with %s\n", command);
                mxFree(command);
                break;
            }
            int num = (int)mxGetScalar(prhs[++k]);
        ...
        //Simulated instrument illumination (FWA),original param 'I'
            ...
            tillum_set = spec = 1;
            if(k + 1 >= nrhs){
                printf("Parameter expected following -I");
                mxFree(command);
                break;
            }
            if(!mxIsNumeric(prhs[k + 1])){
                mwSize param_size = mxGetN(prhs[k + 1]) * sizeof(mxChar);
                char* next_param = (char*)mxMalloc(param_size);
                next_param = mxArrayToString(prhs[k + 1]);
                //if it is not a filename
                if(!isFile(next_param)){
                    ...
                    if(read_xspect(&cust_tillum, &mt, next_param) != 0){
                        printf("Failed to read custom target illuminant spectrum in file '%s'", next_param);
                        mxFree(next_param);
                        mxFree(command);
                        break;
                    }
                    if (mt != inst_mrt_none
                        ...
                        printf("Target illuminant '%s' is wrong measurement type",next_param);
                        mxFree(next_param);
                        mxFree(command);
                        break;
                    }
                ...
                int num = (int)mxGetScalar(prhs[++k]);
                //original parameter 'A' or 'M0'
                if(num == 0){
                    tillum = icxIT_A;
                //original parameter 'C'
                }else if(num == 1){
                    tillum = icxIT_C;
                //original parameter 'D50' or 'M1'
                }else if(num == 2){
                    tillum = icxIT_D50;
                //original parameter 'D50M2' or 'M2'
                }else if(num == 3){
                    tillum = icxIT_D50M2;
                //original parameter 'D65'
                }else if(num == 4){
                    tillum = icxIT_D65;
                //original parameter 'F5'
                }else if(num == 5){
                    tillum = icxIT_F8;
                //original parameter 'F8'
                }else if(num == 6){
                    tillum = icxIT_F8;
                //original parameter 'F10'
                }else if(num == 7){
                    tillum = icxIT_F10;
                }else{
        ...
        //Spectral Illuminant type for XYZ computation,original parameter 'i'
            ...
            if(!mxIsNumeric(prhs[k + 1])){
                ...
                if(!isFile(next_param)){
                    ...
                    tillum = icxIT_custom;
                    if(read_xspect(&cust_tillum, &mt, next_param) != 0){
                        printf("Unable to read custom illuminant file '%s'", next_param);
                        mxFree(next_param);
                        mxFree(command);
                        break;
                    }
                    if (mt != inst_mrt_none
                        ...
                        && mt != inst_mrt_emission_flash
                        && mt != inst_mrt_ambient_flash){
                        printf("Custom illuminant '%s' is wrong measurement type",next_param);
                        mxFree(next_param);
                        mxFree(command);
                        break;
                    }         
                ...
                //original parameter 'C'
                    ...
                    illum = icxIT_C;
                //original parameter 'D50'
                }else if(num == 2){
                    illum = icxIT_D50;
                //original parameter 'D50M2'
                }else if(num == 3){
                    illum = icxIT_D50M2;
                //original parameter 'D65'
                }else if(num == 4){
                ...
                //original parameter 'F10'
                    ...
                    printf("Unrecognised illuminant '%d'", num);
                    mxFree(command);
                    break;
                }
            }
        ...
        //Spectral Observer type,original param 'Q'
            ...
            if(k + 1 >= nrhs){
                printf("Parameter expected following %s\n",command);
                mxFree(command);
                break;
            }
            if(!mxIsNumeric(prhs[k + 1])){
                mwSize param_size = mxGetN(prhs[k]) * sizeof(mxChar);
                char* next_param = (char*)mxMalloc(param_size);
                next_param = mxArrayToString(prhs[++k]);
                obType = icxOT_custom;
				if (read_cmf(custObserver, next_param) != 0){
                    printf("Failed to read custom observer CMF from -Q file '%s'",next_param);
                    mxFree(next_param);
                    mxFree(command);
                    break;
                }
                ...
                int num = (int)mxGetScalar(prhs[++k]);
                if (num == 0) {			/* Classic 2 degree */
                    obType = icxOT_CIE_1931_2;
                } else if (num == 1) {	/* Classic 10 degree */
                    obType = icxOT_CIE_1964_10;
                } else if (num == 2) {		/* Latest 2 degree */
                    obType = icxOT_CIE_2012_2;
                } else if (num == 3) {	/* Latest 10 degree */
                    obType = icxOT_CIE_2012_10;
                } 
#ifndef SALONEINSTLIB
                else if (num == 4) {		/* Stiles and Burch 1955 2 degree */
                    obType = icxOT_Stiles_Burch_2;
                } else if (num == 5) {		/* Judd and Voss 1978 2 degree */
                    obType = icxOT_Judd_Voss_2;
                } else if (num == 6) {		/* Shaw and Fairchilds 1997 2 degree */
        ...
        //Extra filter compensation file,original parameter 'E'
            ...
            if(isFile(next_param)){
                ...
                printf("Parameter expected following -E");
                mxFree(next_param);
                mxFree(command);
                break;
            }
        ...
        //Colorimeter Correction Matrix or Colorimeter Calibration Spectral Samples,original parameter 'X'
            ...
            if(k + 1 >= nrhs){
                ...
                next_param = mxArrayToString(prhs[k + 1]);
                if(!isFile(next_param)){
                    printf("Parameter expected after -X");
                    mxFree(next_param);
                    mxFree(command);
                    break;
                }
        ...
        //Present reference spectrum,original parameter 'R'
            ...
            if(k + 1 >= nrhs){
                ...
                next_param = mxArrayToString(prhs[k + 1]);
                if(!isFile(next_param)){
                    printf("-R fname.sp syntax incorrect\n");
                    mxFree(next_param);
                    mxFree(command);
                    break;
                }
        ...
        //to change
            ...
            if(!mxIsNumeric(prhs[k + 1])){
                ...
                if(next_param[0] == 'R' || next_param[0] == 'r'){
                    //to change
                    if(next_param[1] != ':'){
                        printf("-Y R:rate syntax incorrect");
                        mxFree(next_param);
                        mxFree(command);
                        break;
                    } 
                    ...
                    if(refrate < 5.0 || refrate > 150.0){
                        printf("-Y R:rate %f Hz not in valid range", refrate);
                        mxFree(next_param);
                        mxFree(command);
                        break;
                    }
                    ...
                    if(next_param[1] != ':'){
                        printf("-Y W:fname.sp syntax incorrect");
                        mxFree(next_param);
                        mxFree(command);
                        break;
                    } 
        ...
        //Serial port flow control,original parameter 'W'
            ...
            if(k + 1 >= nrhs){
                printf("Parameter expected following %s", command);
                mxFree(command);
                break;
            }
            if(!mxIsNumeric(prhs[k + 1])){
...
// 		if (inMatrix[fa][0] == '-') {	/* Look for any flags */
// 			char *na = NULL;		/* next argument after flag, null if none */
// 
// 			if (inMatrix[fa][2] != '\000')
// 				na = &inMatrix[fa][2];		/* next is directly after flag */
// 			else {
// 				if ((fa+1+mfa) < nrhs) {
// 					if (inMatrix[fa+1][0] != '-') {
// 						nfa = fa + 1;
// 						na = inMatrix[nfa];		/* next is seperate non-flag argument */
// 					}
// 				}
// 			}
// 
...
// 				}
// 				g_log->debug = debug;
// 			} else if (inMatrix[fa][1] == '?') {
// 				usage("Usage requested");
// 
// 			} else if (inMatrix[fa][1] == 'v') {
// 				verb = 1;
// 				g_log->verb = verb;
// 
// 			} else if (inMatrix[fa][1] == 's') {
// 				pspec = 1;
// 
// #ifndef SALONEINSTLIB
// 			} else if (inMatrix[fa][1] == 'S') {
...
// 			} else if (inMatrix[fa][1] == 'w') {
// 				labwpillum = 1;
// #endif /* !SALONEINSTLIB */
// 
// 			/* Spectral Observer type */
// 			} else if (inMatrix[fa][1] == 'Q') {
// 				fa = nfa;
...
// 
// 				if (inMatrix[fa][2] != '\000') {
// 					if (inMatrix[fa][2] == 'b' || inMatrix[fa][2] == 'B')
// 						emiss = 2;
// 					else if (inMatrix[fa][2] == 'w' || inMatrix[fa][2] == 'W')
// 						emiss = 3;
// 					else
// 						usage("-e modifier '%c' not recognised",inMatrix[fa][2]);
// 				}
// 
// 			/* Request telephoto measurement */
// 			} else if (inMatrix[fa][1] == 'p') {
// 
// 				emiss = 1;
// 				trans = 0;
// 				tele = 1;
...
// 					else
// 						usage("-p modifier '%c' not recognised",inMatrix[fa][2]);
// 				}
// 
// 			/* Request ambient measurement */
// 			} else if (inMatrix[fa][1] == 'a') {
// 				if (trans) {
...
// 
// 				if (inMatrix[fa][2] != '\000') {
// 					if (inMatrix[fa][2] == 'w' || inMatrix[fa][2] == 'W')
// 						refwr = 1;
// 					else
// 						usage("-r modifier '%c' not recognised",inMatrix[fa][2]);
// 				}
// 
// 			/* Filter configuration */
// 			} else if (inMatrix[fa][1] == 'F') {
...
// 
// 			/* Show densities */
// 			} else if (inMatrix[fa][1] == 'd') {
// 				doDensity = 1;
// 
// #endif /* !SALONEINSTLIB */
// 
// 			/* Manual calibration */
// 			} else if (inMatrix[fa][1] == 'K') {
// 
// 				if (na != NULL && na[0] >= '0' && na[0] <= '9') {
// 					docalib = atoi(na);
// 					fa = nfa;
...
// 
// 			/* No auto calibration */
// 			} else if (inMatrix[fa][1] == 'N') {
// 				nocal = 1;
// 
// 			/* Do one cal. or measure and exit */
// 			} else if (inMatrix[fa][1] == 'O') {
// 				doone = 1;
// #ifndef SALONEINSTLIB
// 				if (na != NULL) {
// 					strncpy(outspname,na,MAXNAMEL-1); outspname[MAXNAMEL-1] = '\000';
...
// 
// 			} else 
// 				usage("Flag -%c not recognised",inMatrix[fa][1]);
// 		}
// 		else
// 			break;
// 	}
...
// 				          xmin, xmax, ymin, ymax, 2.0);
		...
		if (sufwa == 0) {		/* Not setting up fwa, so show reading */
			...
			if (ambient && (cap2 & inst2_ambient_mono)) {
                ...
                double tmp[2] = {XYZ[1], Lab[0]};
                double* out = mxMalloc(2 * sizeof(double));
                for(int idx = 0; idx < 2; idx++){
                    out[idx] = tmp[idx];
                }
                mxSetDoubles(plhs[1], out);
			} else {
				if (doYxy) {
					/* Print out the XYZ and Yxy */
					printf("\n Result is XYZ: %f %f %f, Yxy: %f %f %f\n",
					XYZ[0], XYZ[1], XYZ[2], Yxy[0], Yxy[1], Yxy[2]);
                    plhs[1] = mxCreateNumericMatrix(1, 6, mxDOUBLE_CLASS, mxREAL);
                    double tmp[6] = {XYZ[0], XYZ[1], XYZ[2], Yxy[0], Yxy[1], Yxy[2]};
                    double* out = mxMalloc(6 * sizeof(double));
                    ...
                    plhs[1] = mxCreateNumericMatrix(1, 6, mxDOUBLE_CLASS, mxREAL);
                    double tmp[6] = {XYZ[0], XYZ[1], XYZ[2], LCh[0], LCh[1], LCh[2]};
                    double* out = mxMalloc(6 * sizeof(double));
                    for(int idx = 0; idx < 6; idx++){
                        out[idx] = tmp[idx];
                    }
                    ...
                    plhs[1] = mxCreateNumericMatrix(1, 6, mxDOUBLE_CLASS, mxREAL);
                    double tmp[6] = {XYZ[0], XYZ[1], XYZ[2], Yuv[0], Yuv[1], Yuv[2]};
                    double* out = mxMalloc(6 * sizeof(double));
                    for(int idx = 0; idx < 6; idx++){
                        out[idx] = tmp[idx];
                    }
					...
					XYZ[0], XYZ[1], XYZ[2], labwpname, Lab[0], Lab[1], Lab[2]);
                    plhs[1] = mxCreateNumericMatrix(1, 6, mxDOUBLE_CLASS, mxREAL);
                    double tmp[6] = {XYZ[0], XYZ[1], XYZ[2], Lab[0], Lab[1], Lab[2]};
                    double* out = mxMalloc(6 * sizeof(double));
                    for(int idx = 0; idx < 6; idx++){
                        out[idx] = tmp[idx];
                    }
